...
reload(lfd); reload(a1_pca_perBrand)
...
# ────────────────────────────────────────────────────────────
# 2) PCA - coeff, score 계산
# ────────────────────────────────────────────────────────────
# s06_pca.a1_pca_perBrand

# ────────────────────────────────────────────────────────────
# 3) 군집분석 w pca score (군집라벨, 중심 산출)
# ────────────────────────────────────────────────────────────
def labeling_cluster_on_pca(pca_score_w_meta_cols, k, apply_stdscaler=False, use_cosine=False, batch=True):
    """
    Parameters
    ----------
    pca_score_w_meta_cols : create_mds_perBrand(...)의 결과 (name, dim1, dim2 포함)
    k : 클러스터 수 (반드시 지정)
    ...
    # 데이터 전처리
    #----------------------------
    df = pca_score_w_meta_cols.set_index('name')
    ...
    model = Model(
        ...
        n_init="auto",
        random_state=42,
        **({"batch_size": 1024} if batch else {})
    )

    labels = model.fit_predict(X_scaled)
    centroids = model.cluster_centers_  # shape = (k, 2) — MDS 좌표계의 중심

    #----------------------------
    # 라벨 부착
    #----------------------------
    data_labeled = pca_score_w_meta_cols.copy()
...
# ────────────────────────────────────────────────────────────
# 4) pca+군집 시각화 (Perceptual Map) 
# ────────────────────────────────────────────────────────────
def pmap_w_pca_clustered(
    pca_w_cluster, centroids_df, size_col,
    apply_stdscaler, apply_l2, input_data_filtering_conditions # pca 계산조건 표시용 (그래프 작성에는 사용하지 않음)
    ):

    df = pca_w_cluster.copy()
    ...
    fig.update_layout(
        title=dict(
            text=f'Brand Perceptual Map via PCA (Clustered)<br><span style="font-size:11px">{subtitle}</span>',
            x=0.05
        ),
        xaxis_title="PC1",
        yaxis_title="PC2",
...
if __name__ == '__main__':
    ...
    # 3) 군집분석 w pca score (군집라벨, 중심 산출)
    pca_w_cluster, centroids_df = labeling_cluster_on_pca(pca_score_w_meta_cols, k=4)

    # 4) pca+군집 시각화 (Perceptual Map) 
    size_col='review_count'
    fig = pmap_w_pca_clustered(
        pca_w_cluster, centroids_df, size_col,
        apply_stdscaler, apply_l2, input_data_filtering_conditions # pca 계산조건 표시용 (그래프 작성에는 사용하지 않음)
        )
    fig.show()
